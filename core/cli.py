...
def set_current_session(user, access_token):
...
def login_user():
    ...
    try:
        ...
        set_current_session(user, tokens["access_token"])
...
def list_reminders():
    ...
    try:
        ...
        logger.exception("Error list reminder")  # stack trace va al log
...
def delete_reminder():
    ...
    try:
        from infrastructure.repositories import ReminderRepository
        repo = ReminderRepository(db_session)
        success = ReminderService.delete_reminder(current_user, reminder_id, reminder_repo=repo)
        if success:
        ...
        logger.exception("Error delete reminder")  # stack trace va al log
        safe_print("Failed to delete reminder. Please check your input or try again later.")
