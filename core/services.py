...
def parse_expiration(amount: int, unit: str) -> int:
    ...
    if unit == "minutes":
        ...
        raise ValueError("Invalid unit, use 'minutes', 'hours' or 'days'")

    if minutes > max_expiration:
        raise ValueError("Reminder cannot exceed 7 days")
    if minutes < 1:
        raise ValueError("Reminder must last at least 1 minute")
