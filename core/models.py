...
class Reminder:
    ...
    def __init__(self, owner_id, text: str, expires_in_minutes: int):
        if len(text) > self.MAX_TEXT_LENGTH:
            raise ValueError(f"The reminder can't exceed {self.MAX_TEXT_LENGTH} characters.")
        
        self.owner_id = owner_id
        self.text = text
        self.created_at = datetime.now()

        max_expiration = timedelta(minutes=self.MAX_EXPIRATION_DAYS)
        requested_expiration = timedelta(minutes=expires_in_minutes)

        if requested_expiration > max_expiration:
            requested_expiration = max_expiration
